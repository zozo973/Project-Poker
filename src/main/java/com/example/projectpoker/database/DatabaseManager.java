...
public class DatabaseManager {
    ...
    public static void finalizeGameSession(int gameSessionId, User user, Game game, Player player) {
        ...
        userDAO.updateCurrentBalance(storedUser.getId(), player.getBalance());
        if (wonGame) {
            userDAO.incrementTotalWins(storedUser.getId());
        }

        User updatedUser = userDAO.getById(storedUser.getId());
        if (updatedUser != null) {
            user.setCurrentBalance(updatedUser.getCurrentBalance());
            user.setTotalHandsPlayed(updatedUser.getTotalHandsPlayed());
            user.setTotalWins(updatedUser.getTotalWins());
        }
